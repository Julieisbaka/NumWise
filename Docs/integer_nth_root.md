...
- **`0.2.7`** — Shares a capped exponentiation-by-squaring helper between
 arbitrary-degree Newton iteration and exact power correction, reducing
 repeated BigInt multiplications.
...
exact power comparisons using capped exponentiation by squaring, starting from
a `Math.pow` estimate and correcting
